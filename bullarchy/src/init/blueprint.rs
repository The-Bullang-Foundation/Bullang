...
> This file was generated by `bullarchy init` and is **automatically deleted** when
> you run `bullarchy convert`. Keep it as a reference while you write your project,
...
Run `bullarchy stdlib` for full signatures and descriptions.
...
```sh
bullarchy convert my_project       # engine -> rs, pipeline -> py
bullarchy convert my_project -e rs # ERROR: project uses multiple languages
```
...
```sh
bullarchy fmt              # rewrite all .bu files to canonical style
bullarchy fmt --dry-run    # show what would change, write nothing
bullarchy check            # validate + type-check + report formatting drift
```
...
```sh
bullarchy init game_engine --blueprint game_engine.bu
```
...
|---------------------------------------------|----------------------------------------------------------|
| `bullarchy init <name>`                       | Scaffold a new project (depth 2 by default)              |
| `bullarchy init <name> --depth N`             | Scaffold with a custom hierarchy depth (1–6)             |
| `bullarchy init <name> --lang <ext>`          | Set the target language at init time                     |
| `bullarchy init <name> --blueprint <file>`    | Init from a blueprint file                               |
| `bullarchy convert <folder>`                  | Transpile (reads #lang from inventory, default: rs)      |
| `bullarchy convert <folder> -e rs`            | Override target language                                 |
| `bullarchy convert file.bu`                 | Transpile a single file — output written next to source  |
| `bullarchy convert file.bu -e rs -o out.rs` | Transpile a single file to an explicit path              |
| `bullarchy fmt`                             | Format all .bu files to canonical style                  |
| `bullarchy fmt --dry-run`                   | Show what would change without writing                   |
| `bullarchy check`                           | Validate, type-check, and verify formatting              |
| `bullarchy stdlib`                          | List all builtin functions with signatures               |
| `bullarchy update`                          | Pull and reinstall from main branch                      |

| `bullarchy lsp`                             | Start the LSP server                                     |
| `bullarchy editor-setup`                    | Write LSP config for Neovim / Vim / Helix / Emacs / Zed  |
...
```sh
# 1. Create a project
bullarchy init my_project --depth 3 --lang rs
